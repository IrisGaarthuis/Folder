...
### **NEW** Histogram of tempi 
```{r, echo = FALSE}
tempi <- genre %>%
  mutate(mode = ifelse(mode == 0, "Minor", "Major")) %>%
  ggplot(aes(x = tempo, fill = mode)) +
  geom_histogram(position = "dodge", binwidth = 7) +
  facet_wrap(~category) +
  theme_light() +
  labs(
    title = "Histogram of tempi for movie genres",
    fill = "Mode",
    x = "Tempo (BPM)")
...
On average, feelgood/comedy movies have a higher BPM than the other three genres (+/- 120 BPM). The average BPM for horror movies is the lowest (around 90 BPM). The distribution for romance/drama and action is about the same. 
...
keys <- genre %>%
  ...
  theme_light() +
  labs(
    title = "Histogram of keys for movie genres",
    fill = "Mode",
    y = "Count",
    x = "Key"
  ) +
...
A clear distinction between horror & action and romance/drama & feelgood/comedy: the first two genres contain a lot of tracks in the C#-key. However, the other two genres do not contain as many C#-key tracks.
...
film_music <-
  bind_rows(comedy %>% mutate(genre = "Feelgood-Comedy"),
...
slider <- filter_slider(id = "valence", label = "Valence", sharedData = shared_genre, "Valence", column = ~valence, step = 1)

checkbox1 <- filter_checkbox(id = "track.album.name", label = "Movie", sharedData = shared_genre, group = ~track.album.name, inline = TRUE)

checkbox2<- filter_checkbox(id = "category", label = "Genre", sharedData = shared_genre, group = ~category, inline = TRUE)

#checkbox3 <- filter_checkbox(id = "track.album.name", label = "Horror", sharedData = shared_genre, group = ~horror_movies)
...
#  ) %>%
 p1 <- ggplot(shared_genre, 
  ...
  geom_text(                 
    ...
    geom_vline(alpha = 0.4,
    aes(xintercept = 0.5)
    ) +
  geom_hline(alpha = 0.4, aes(
    yintercept = 0.5)
    ) +
  #facet_wrap(~category
  #  ) +   
...
***
`r bscols(list(slider, checkbox2, checkbox1))`
